...
int main() {
    ...
    list <no> adj[10];                // Vetor de listas de adjacências

    cin >> nVertices >> s;            // Lê o número de vértices e o vértice de partida

    cin >> u >> v >> peso;
    while (u != -1 && v != -1 && peso != -1) {
        cria_aresta(adj, u, v, peso, orientado);             // Cria as arestas com base nas entradas fornecidas
        cin >> u >> v >> peso;                               // Lê as próximas entradas
    }
    ...
    i = 0;
    while (!adj[i].empty()) {
        adj[i].pop_front();                                  // Limpa as listas de adjacências, removendo todas as arestas
        i++;
    }
